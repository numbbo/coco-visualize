...
def ertvalues(results: ResultSet,
              indicator: str, 
    ...
    for algo, algo_results in indicator_results.by_algorithm():
        ...
        ecdf = stats.ecdf(stats.CensoredData(runtimes["__fevals_dim"].filter(runtimes["__target_hit"] > 0).to_numpy(), 
                                             right=runtimes["__fevals_dim"].filter(runtimes["__target_hit"] == 0).to_numpy())).cdf
